...
import 'package:ruet_cse/screens/home_drawer.dart';
...
class _alumniState extends State<alumni> {
  ...
  Future upload_image() async {
    Reference ref =
        FirebaseStorage.instance.ref().child('cse/alumni/$doc_time');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Alumni", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          if (FirebaseAuth.instance.currentUser == null) {
            Fluttertoast.showToast(msg: 'Please log in to add Alumni');
          } else {
            showDialog(
                context: context,
...
class alumni_widget extends StatefulWidget {
  const alumni_widget({super.key});

  @override
  State<alumni_widget> createState() => _alumni_widgetState();
}

class _alumni_widgetState extends State<alumni_widget> {
  Timestamp? timestamp;
  ...
  Future upload_image() async {
    Reference ref =
        FirebaseStorage.instance.ref().child('cse/alumni/$timestamp');
    await ref.putFile(_image!);
    download_url = await ref.getDownloadURL();
  }
                  ...
                  child: ListTile(
                    onLongPress: () {
                      ...
                      if (FirebaseAuth.instance.currentUser != null) {
                        ...
                        Fluttertoast.showToast(
                            msg: 'Please log in to edit Alumni');
                      }
